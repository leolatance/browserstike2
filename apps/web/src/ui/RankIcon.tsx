...
export function RankIcon({ mmr, size = 28, withName = false }: { mmr: number; size?: number; withName?: boolean }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, verticalAlign: 'middle', lineHeight: 1 }} title={`${r.name} · ${mmr} MMR`}>
      <svg width={size} height={size} viewBox="0 0 32 32" aria-label={r.name} role="img" style={{ display: 'block', flex: '0 0 auto' }}>
